...
* **Hard negatives.**  A quarter of the set has an all-zero mask, and most of
  those still contain something smoke-*like*: brown dust clouds, soft
  vehicle-underbody shadows, or motion blur.  Without these the network learns
...
NEGATIVE_KINDS = {"dust": 0.32, "shadow": 0.26, "motion_blur": 0.20, "plain": 0.22}
...
def make_plume_alpha(
    ...
    *solved* so that ``(alpha > MASK_ALPHA_CUTOFF)`` covers approximately
    ...
    Args:
        ...
        opacity_range: Allowed peak alpha.
    ...
    opacity = float(np.clip(opacity, *opacity_range))
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--train", type=int, default=1600, help="number of training samples")
    parser.add_argument("--val", type=int, default=400, help="number of validation samples")
